...
def creat_bounding_box_file(bbox_max, bbox_min, dim_x, dim_y, dim_z):
    ...
    f.write(str(bbox_min[0] + dim_x) + ' ' + str(bbox_min[1]) + ' ' + str(
        bbox_min[2]) + '\n')  # 1 100 xmin + dimx, ymin, zmin
    f.write(str(bbox_min[0] + dim_x) + ' ' + str(bbox_min[1] + dim_y) + ' ' + str(
        bbox_min[2]) + '\n')  # 2 110 xmin, ymin + dimy, zmin
    f.write(str(bbox_min[0]) + ' ' + str(bbox_min[1] + dim_y) + ' ' + str(
        bbox_min[2]) + '\n')  # 3 010 xmin = dimx, ymin + dimy, zmin

    # 4 upper vertexs
    f.write(str(bbox_max[0]) + ' ' + str(bbox_max[1]) + ' ' + str(bbox_max[2]) + '\n')  # 4 111 xmax, ymax, zmax
    f.write(str(bbox_max[0] - dim_x) + ' ' + str(bbox_max[1]) + ' ' + str(
        bbox_max[2]) + '\n')  # 5 011 xmax - dimx, ymax, zmax
    f.write(str(bbox_max[0] - dim_x) + ' ' + str(bbox_max[1] - dim_y) + ' ' + str(
        bbox_max[2]) + '\n')  # 6 001 xmax, ymax - dimy, zmax
    f.write(str(bbox_max[0]) + ' ' + str(bbox_max[1] - dim_y) + ' ' + str(
